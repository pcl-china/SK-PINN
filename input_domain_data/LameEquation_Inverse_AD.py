...
class PhysicsInformedNN():
    # 初始化
    def __init__(self, x_area,X_leftright, Sxx_leftright, X_topbottom, Syy_topbottom, E_Training,
                 layers, layers_S, layers_Lame, ub, lb, Adam_iter, LBFGS_iter):
    ...
    def Calculate_loss(self):
        ...
        loss = 1.0 * loss_bc_leftright + 1.0 * loss_bc_topbottom + \
               1.0 * loss_fe1 + 1.0 * loss_fe2 + 1.0 * loss_fe3 + \
               1.0 * loss_fe4 + 1.0 * loss_fe5
        self.iter += 1
        if self.iter % 100 == 0:
            print(
...
model = PhysicsInformedNN(x_area,x_leftright,Sxx_leftright,x_topbottom,Syy_topbottom,E_Training,
                          layers,layers_S,layers_Lame,ub,lb,Adam_iter,LBFGS_iter)
...
plt.colorbar()
plt.figure(figsize=(3, 3))
plt.scatter(x_area[:, 0], x_area[:, 1],c=Poissons_GroundTruth.reshape(-1,1)-v/(1-v),s=5, cmap='jet')
plt.colorbar()
plt.figure(figsize=(3, 3))
