...
class PhysicsInformedNN():
    # 初始化
    def __init__(self, x_area,X_bc_inflow,u_bc_inflow,X_bc_outflow,X_bc_noslip,Re,B,
        ...
        self.lbdf1 = self.lbdf2 = self.lbdf3 = self.lbdubc_in = self.lbdvbc_in = self.lbdbc_out = self.lbdubc_noslip = self.lbdvbc_noslip = 1.0
...
Adam_iter = 20000
LBFGS_iter = 40000
model = PhysicsInformedNN(x_area,X_bc_inflow,u_bc_inflow,X_bc_outflow,X_bc_noslip,Re,B,
                          layers,layers_u,layers_v,layers_p,ub,lb,Adam_iter,LBFGS_iter)
...
# with pd.ExcelWriter('datamAD.xlsx') as writer:
...
# plt.scatter(x_area[:, 0], x_area[:, 1],c=V_P[:, 0],s=2, cmap='jet')
# plt.xlim(0, 2)
# plt.ylim(0, 1)
# plt.colorbar()
# plt.axis('equal')
...
#
# plt.figure(figsize=(12, 5))
# plt.scatter(x_area[:, 0], x_area[:, 1],c=p_pred[:, 0],s=2, cmap='jet')
# plt.xlim(0, 2)
# plt.ylim(0, 1)
# plt.colorbar()
# plt.axis('equal')
