...
order = 4
warmup_epochs = 30000
T_epoch = (warmup_epochs-10000)//xt_area.shape[1]
Adam_iter = 40000
## 0.5s-3.0s使用这个初始条件
...
for i in range(N):
    ...
    model = PhysicsInformedNN(xt_area, neighborhoods, distances, distance_vectors,h,order,
                    x_ic,uvpc_ic,x_bc1,x_bc2, dxdy, layers,layers_u,layers_v,layers_p,layers_C,T_epoch,warmup_epochs,Adam_iter)
    model.dnn.load_state_dict(torch.load(f"modelDNNSK0304_{i}.pth"))
    print("网络参数已加载！")
    # start = time.perf_counter()
    # model.train()
    # end = time.perf_counter()
    # print("训练时间为", round(end - start), 'seconds')
    # torch.save(model.dnn.state_dict(), f"modelDNNSK0304_{i+1}.pth")
    # print("网络参数已保存！")
    # uvpc_f = model.predict(x_f)
    # np.save(f"uvpc_{i+1}.npy", uvpc_f)
...
# file_names = [f"uvpc_{i}.npy" for i in range(N+1)]
