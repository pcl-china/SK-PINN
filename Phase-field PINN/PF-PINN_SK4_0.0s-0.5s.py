...
class PhysicsInformedNN():
    ...
    def __init__(self, xt_area, neighborhoods, distances, distance_vectors,h,order,
                 x_ic,u_ic,x_bc1,x_bc2, dxdy, layers,layers_u,layers_v,layers_p,layers_C,T_epoch,warmup_epochs,Adam_iter):
    ...
    def Calculate_loss(self,x_ic_batch, u_ic_batch, x_bc1_batch, x_bc2_batch, xt_area_batch, i_batch, j_batch):
        ...
        if self.iter % 100 == 0:
            print(
                'Iter %d, lr: %.5e, Loss: %.5e, loss_icC: %.5e, loss_icuv: %.5e, loss_bc1: %.5e, loss_bc2u: %.5e, loss_bc2dvdx: %.5e, loss_fe1: %.5e, Loss_fe2: %.5e, Loss_fe3: %.5e, Loss_fe4: %.5e\n'
                #
                % (self.iter, self.scheduler.get_last_lr()[0], loss.item(), loss_icC.item(), loss_icuv.item(), loss_bc1.item(), loss_bc2u.item(), loss_bc2dvdx.item(), loss_fe1.item(), loss_fe2.item(), loss_fe3.item(), loss_fe4.item())
                # )
            )
...
u_ic = np.tanh(((x_ic[:,0:1]**2+(x_ic[:,1:2]+0.5)**2)**.5-0.25)/(2**.5*0.01))
## 0.5s之后使用这个初始条件
...
order = 4
warmup_epochs = 20000
T_epoch = warmup_epochs//xt_area.shape[1]
Adam_iter = 40000
model = PhysicsInformedNN(xt_area, neighborhoods, distances, distance_vectors,h,order,
                 x_ic,u_ic,x_bc1,x_bc2, dxdy, layers,layers_u,layers_v,layers_p,layers_C,T_epoch,warmup_epochs,Adam_iter)

# model.dnn.load_state_dict(torch.load("modelDNNSK0304_0.pth"))
# print("网络参数已加载！")
...
# torch.save(model.dnn.state_dict(), "modelDNNSK0304_0.pth")
# print("网络参数已保存！")

# x = np.arange(-0.5+0.0*dx,0.5+1.0*dx,5*dx)
# y = np.arange(-1.0+0.0*dy,1.0+1.0*dy,5*dy)
# t = np.arange(0.0*dt, TT + 1.0*dt, 10*dt)
# X, Y, T = np.meshgrid(x, y, t)
#
# X_flat = X.flatten()
# Y_flat = Y.flatten()
...
# fig = plt.figure()
# ax = fig.add_subplot(111, projection='3d')
# scatter = ax.scatter(xt_area_[:,0], xt_area_[:,1], xt_area_[:,2], c=uvpc_xt_area[:,2], cmap='jet', marker='o')
# cbar = fig.colorbar(scatter, ax=ax)
...
#     u_pred,v_pred,c_pred,p_pred = model.predict(xt_area[:,i,:])
#     v_pred = v_pred
#     c_pred = c_pred
#     p_pred = p_pred
#     yc[i] = np.sum(xt_area[:,i,1:2]*(1-c_pred)/2)/np.sum((1-c_pred)/2)+1.0
#     vc[i] = np.sum(v_pred*(1-c_pred)/2)/np.sum((1-c_pred)/2)
#
# # 创建图形
# plt.figure()
# plt.plot(t, yc)
# plt.xlim(0, 1)  # 设置X轴范围从0到5
# plt.ylim(0.5, 0.7) # 设置Y轴范围从0到30
# # plt.title('质心纵坐标')
# plt.xlabel('t(s)')
